...
const RedIcon: FC<RedIconProps> =  ({
	...
	return (
		<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill={fill} viewBox="0 0 16 16" className={className} style={style}>
			{icon.paths.map((path: PathData, k) => <Path key={k} data={path} />)}
			{icon.symbols.map((symbol: SymbolData, k) => <Symbol key={k} data={symbol} />)}
			{icon.circles.map((circle: CircleData, k) => <Circle key={k} data={circle} />)}
			{icon.rects.map((rect: RectData, k) => <Rect key={k} data={rect} />)}
		</svg>
	);
};
